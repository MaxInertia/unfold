...
**SHIPPED 2026-08-19 (`feat/served-surface`)** — the manifest stopped being the *only* way to state a surface, which it had quietly become: with two repos open and no `microservice.yaml`, nothing published "this repo serves this key" and every cross-repo call resolved to nothing. Tier 1 of the plan above ("route tables are already in the source — `mux.HandleFunc`, gRPC registration") now includes the gRPC half: `<Service>_ServiceDesc` carries the service name and its RPCs, so both ends of an edge come from generated code. Tier 3 ("hand-declared, only what neither knows") is `serves` in the recognizer files — any kind of key, `svc.Service/*` for a whole service, and the only tier that costs no index. See `~/obsidian-vault/docs/2026-08-19-unfold-declaring-apis.md`.
